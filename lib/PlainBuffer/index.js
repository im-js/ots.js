...
class PlainBuffer {
    static getCellCheckum (pk, value) {
    ...
    static buildPrimayKey(pk, value) {
        // 需要做内存检查，预估数组大小
        ...
        let cellChecksum = PlainBuffer.getCellCheckum(pk, value);
        buf.writeUInt8(cellChecksum, pos);
        pos += PB_LENGTH.CHECKSUM;

        // row 检验
        buf.writeUInt8(PB_TAG.ROW_CHECKSUM.value, pos);
        pos += PB_TAG.ROW_CHECKSUM.length;
        let rowChecksum = checksum.crc8(0x00, cellChecksum);
        // 没有deleteMarker, 要与0x0做crc.
        rowChecksum = checksum.crc8(rowChecksum, 0x00);
        buf.writeUInt8(rowChecksum, pos);
        pos += PB_LENGTH.CHECKSUM;
